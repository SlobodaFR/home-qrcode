...
export class HttpOAuthClient extends OAuthClient {
  ...
  constructor(private readonly config: ConfigService) {
    super();
    this.authBaseUrl = this.config.getOrThrow<string>('AUTH_BASE_URL');
  ...
  authorizeUrl(redirectUri: string): string {
    const url = new URL('/authorize', this.authBaseUrl);
  ...
  async fetchUserInfo(accessToken: string): Promise<UserProfile> {
    const response = await fetch(new URL('/userinfo', this.authBaseUrl), {
  ...
  private async requestToken(params: Record<string, string>): Promise<TokenPair> {
    const response = await fetch(new URL('/token', this.authBaseUrl), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ client_id: this.clientId, client_secret: this.clientSecret, ...params }),
    });
    if (!response.ok) {
